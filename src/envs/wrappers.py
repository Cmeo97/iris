...
def make_rs(
    ...
    return RobosuiteIRISWrapper(env)


class ActionSpace:
    def __init__(self, act_space):
        self.n = len(act_space["action"].high)


class RobosuiteIRISWrapper(Wrapper):
    def __init__(self, env: PandaRoboSuite) -> None:
        super().__init__(env)
        self.action_space = ActionSpace(env.act_space)

    def reset(self):
        obs_dict = self.env.reset()
        return obs_dict["rgb"].transpose(1, 2, 0)

    def step(self, action):
        obs_dict = self.env.step(action)
        obs = obs_dict["rgb"].transpose(1, 2, 0)
        reward = obs_dict["reward"]
        done = obs_dict["is_last"]
