...
class ActTokenizer(nn.Module):
    def __init__(
        self,
        act_dims: int
    ) -> None:
        super().__init__()
        self.act_encoder = nn.Sequential(nn.Linear(act_dims, 1), nn.Sigmoid())
        self.act_decoding = nn.Sequential(nn.Linear(1, act_dims), nn.Sigmoid())
    
    def __repr__(self) -> str:
        return "act_tokenizer"
    
    def forward(self, x):
        z = self.act_encoder(x)
        rec = self.act_decoding(z)
        return  z, rec 
    
    def encode(self, x):
        return self.act_encoder(x)
    
    def compute_loss(self, batch: Batch, **kwargs: Any):
        _, rec = self(batch["actions"])
        rec_loss = torch.abs(batch["actions"] - rec).mean()
        return LossWithIntermediateLosses(
