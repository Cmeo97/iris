...
class ActorCritic(nn.Module):
    def __init__(self, act_dims, use_original_obs: bool = False) -> None:
        super().__init__()
        ...
        self.critic_linear = nn.Linear(512, 1) # TODO: check which should be the output of the critic + dimension
        self.actor_linear = ActorOutput(512, act_dims, dist="trunc_normal")
    ...
    def compute_loss(
        ...
        **kwargs: Any
        ...
        outputs = self.imagine(batch, obs_tokenizer, act_tokenizer, world_model, horizon=imagine_horizon)
        ...
        d = outputs.dist_actions
        log_probs = d.log_prob(outputs.actions)[:, :-1]
        loss_actions = -1 * (log_probs.sum(-1) * (lambda_returns - values.detach())).mean()
    ...
    def imagine(
        ...
        wm_env = WorldModelEnv(obs_tokenizer, act_tokenizer, world_model, device)
        ...
        for k in tqdm(
            ...
            outputs_ac = self(obs)
            act = outputs_ac.dist_actions.sample()

            obs, reward, done, _ = wm_env.step(
                act, should_predict_next_obs=(k < horizon - 1)
            )

            all_actions.append(act.unsqueeze(1))
            all_dist_actions.append(outputs_ac.dist_actions)
        ...
        loc = []
        scale = []
        dist_method = type(all_dist_actions[0])
         
        for dist in all_dist_actions: 
            loc.append(getattr(dist, 'loc').unsqueeze(1))
            scale.append(getattr(dist, 'scale').unsqueeze(1))

        dist_actions = dist_method(torch.cat(loc, dim=1), torch.cat(scale, dim=1))
