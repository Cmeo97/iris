...
class ActorCritic(nn.Module):
    ...
    def reset(self, n: int, burnin_observations: Optional[torch.Tensor] = None, mask_padding: Optional[torch.Tensor] = None) -> None:
        ...
        dtype = burnin_observations.dtype if burnin_observations is not None else torch.float16
        self.hx = torch.zeros(n, self.lstm_dim, device=device, dtype=dtype, requires_grad=True)
        self.cx = torch.zeros(n, self.lstm_dim, device=device, dtype=dtype, requires_grad=True)
