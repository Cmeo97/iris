...
class Transformer(nn.Module):
    def __init__(self, embed_dim, num_heads, num_layers, mlp_dim, dropout, attention_dropout, 
                 vocab_size=None, vocab_dim=None, shape=None, pos_embed_type='absolute',
        ...
        if self.use_fc_in:
            self.fc_in = nn.Linear(self.vocab_dim, self.embed_dim)
    ...
    def forward(self, inputs, mask=None, deterministic=False, cond=None):
        x = inputs
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, inputs, mask=None):
        ...
        y = self.mlp(y)
        y = self.dropout_y(y, self.training)
...
class BroadcastPositionBiases(nn.Module):
    def __init__(self, shape=None, dtype=torch.float32):
    ...
    def forward(self, x):
        ...
        embs = [
            nn.Parameter(torch.randn(shape[i], chunk_sizes[i], dtype=self.dtype) * 0.02)
            for i in range(n_dim)
        ]
